...
rigaax_intg errorNumber;				/* Run-time error number = 0 by default (ANSI) */

/* External objects */
extern rigaax_intg line; /* Source code line numbers - defined in scanner.c */
extern Token tokenizer(rigaax_void);
 ...
 * -------------------------------------------------------------
 */
rigaax_void printScannerError(rigaax_strg fmt, ...);
rigaax_void displayScanner(BufferPointer ptrBuffer);
rigaax_long getScannerFilesize(rigaax_strg fname);
...
rigaax_intg main3Scanner(rigaax_intg argc, rigaax_strg* argv) {

	BufferPointer sourceBuffer;		/* Pointer to input (source) buffer */
	Token currentToken;				/* Token produced by the scanner */
	rigaax_intg loadSize = 0;		/* The size of the file loaded in the buffer */
	...
	if (argc <= 2) {
		...
		printScannerError("%s", "Usage: <Option=3> <SourceFile>");
		exit(EXIT_FAILURE);
	}
